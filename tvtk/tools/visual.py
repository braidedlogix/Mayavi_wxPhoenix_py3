...
class Animator(HasTraits):
    ...
    time_period = Range(
        1, 100000, 100, desc='Specifies frequency with which timer is called')
    ...
    event_group = Group(
        Item(
            'start_animation', style='simple'),
        Item('_'),
        Item(
            'stop_animation', style='simple'),
        Item('_'),
        show_labels=False, )
...
class MVector(numpy.ndarray):
    ...
    def __new__(subtype, x=0.0, y=0.0, z=0.0):
        ...
        ret = numpy.ndarray.__new__(
            subtype, shape=(3, ), buffer=data, dtype=float, order=False)
        return ret.copy()
...
class Curve(HasTraits):
    ...
    points = Array(
        dtype=float, shape=(None, 3), desc='the points of the curve')
    ...
    actor = Instance(
        tvtk.Actor,
        args=())  # tvtk Actor, for the usual pipeline architecture.
    ...
    def __init__(self, **traits):
        ...
        if axis is not None:
            self._axis_changed(
                numpy.array((1.0, 0.0, 0.0)), numpy.asarray(
                    axis, dtype=float))
    ...
    def rotate(self, angle, axis, origin=numpy.array([0.0, 0.0, 0.0])):
        ...
        p, pi, ax = rotate(axis, angle, origin, self.pos, self.points,
...
class Cone(HasTraits):
    ...
    def _create_points(self, r, h, c, d):
        cs = tvtk.ConeSource(
            radius=r, height=h, center=tuple(c), direction=tuple(d))
        cs.update()
        ps = cs.output
    ...
    def _radius_changed(self):
        points, lines = self._create_points(self.radius, self.height, self.pos,
                                            self.axis)
...
class Sphere(HasTraits):
    ...
    def _create_points(self, r, c):
        sp = tvtk.SphereSource(
            radius=r, center=tuple(c), phi_resolution=20, theta_resolution=20)
        sp.update()
        ps = sp.output
...
class Cylinder(HasTraits):
    ...
    def _create_points(self, r, c, h):
        ...
        points = axis_changed(
            ...
            numpy.array([1.0, 0.0, 0.0]), numpy.array([0.0, 0.0, 0.0]), points)
    ...
    def _radius_changed(self, old, new):
        self.points, polys = self._create_points(self.radius, self.pos,
                                                 self.length)
        self.polydata.points = self.points
        self.polydata.modified()
        self.normals.update()
...
class Box(HasTraits):
    ...
    def _create_points(self, s, c):
        cp = tvtk.CubeSource(
            x_length=s[0], y_length=s[1], z_length=s[2], center=tuple(c))
        cp.update()
        ps = cp.output
        points = ps.points.to_array()
...
class Arrow(HasTraits):
    ...
    radius_cone = Range(
        0.0, 10.0, value=0.08, desc='the radius of cone portion of arrow')
    radius_shaft = Range(
        0.0, 5.0, value=0.03, desc='the radius of shaft portion of arrow')
    ...
    def __init__(self, **traits):
        ...
        self._create_points(self.radius_cone, self.length_cone,
                            self.radius_shaft, self.pos)
        self._color_changed(self.color)
        self._visibility_changed(self.visibility)
        self._x_changed(self.x)
        self._y_changed(self.y)
    ...
    def _radius_cone_changed(self, old, new):
        self.points, polys = self._create_points(
            self.radius_cone, self.length_cone, self.radius_shaft, self.pos)
        self.polydata.points = self.points
        self.polydata.modified()
        self.normals.update()
        self.render()
