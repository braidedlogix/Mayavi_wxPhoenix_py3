...
class GradientControl(tk.Frame):
    ...
    def __init__(self, master, gradient_table, width, height):
        ...
        self.canvas = tk.Canvas(
            self, background="white", width=width, height=height)
        self.canvas.pack()
        self.update()
...
class FunctionControl(tk.Frame):
    ...
    class Channel:
        def __init__(self, function_control, name, color_string, channel_index,
                     channel_mode):
        ...
        def get_value_index(self, color):
            ...
            return int(1 + (self.control.height - 1) * (1.0 - self.get_value(
                color)))

        def get_index_value(self, y):
            """Get value in [0..1] of height index y"""
            return min(
                1.0, max(0.0, 1.0 - float(y - 1) / (self.control.height - 1)))
        ...
        def set_value(self, color, new_value_on_this_channel):
            ...
            if (self.mode == 'hsv'):
                hsva = [
                    color.get_hsva()[0], color.get_hsva()[1],
                ...
                rgba = [
                    color.get_rgba()[0], color.get_rgba()[1],
...
class GradientEditor(tk.Toplevel):
    ...
    def __init__(self, master, vtk_table, on_change_color_table=None):
        ...
        self.function_control_rgb = FunctionControl(
            self, self.gradient_table, "rgb", channel_function_width,
            channel_function_height, on_gradient_table_changed)
        ...
        for it in [("hue", 2), ("sat", 3), ("val", 4), ("alp", 5)]:
            control = FunctionControl(
                self, self.gradient_table, it[0][0], channel_function_width,
                channel_function_height, on_gradient_table_changed)
        ...
        save_button = tk.Button(
            button_frame, text="save", command=self.save_gradient)
        save_button.grid(row=0, column=4)
        load_button = tk.Button(
            button_frame, text="load", command=self.load_gradient)
    ...
    def save_gradient(self):
        ...
        file_name = tkFileDialog.asksaveasfilename(
            defaultextension=".grad", filetypes=filetypes)
        if file_name:
...
if __name__ == "__main__":
    ...
    render_frame.pack()
    render_widget = vtkTkRenderWidget.vtkTkRenderWidget(
        render_frame, width=512, height=512)
