...
class CompositeDataWriter(DataWriter):
    """
    CompositeDataWriter - legacy VTK file writer for
    CompositeDataSet subclasses.
    
    Superclass: DataWriter
    
    CompositeDataWriter is a writer for writing legacy VTK files for
    CompositeDataSet and subclasses.
    @warning
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkCompositeDataWriter, obj, update, **traits)
    ...
    def get_input(self, *args):
        ...
        C++: CompositeDataSet *GetInput(int port)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(CompositeDataWriter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit CompositeDataWriter properties', scrollable=True, resizable=True,
