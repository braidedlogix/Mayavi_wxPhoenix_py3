...
class OSPRayVolumeInterface(VolumeMapper):
    """
    OSPRayVolumeInterface - Removes link dependence on optional ospray
    module.
    
    Superclass: VolumeMapper
    
    Class allows smart_volume to use OSPRay for rendering when ospray is
    enabled. When disabled, this class does nothing but return a warning.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkOSPRayVolumeInterface, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(OSPRayVolumeInterface, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit OSPRayVolumeInterface properties', scrollable=True, resizable=True,
