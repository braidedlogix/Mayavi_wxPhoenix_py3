...
class InterpolatingSubdivisionFilter(PolyDataAlgorithm):
    """
    InterpolatingSubdivisionFilter - generate a subdivision surface
    using an Interpolating Scheme
    
    Superclass: PolyDataAlgorithm
    
    InterpolatingSubdivisionFilter is an abstract class that defines
    ...
    @sa
    LinearSubdivisionFilter ButterflySubdivisionFilter
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkInterpolatingSubdivisionFilter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(InterpolatingSubdivisionFilter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit InterpolatingSubdivisionFilter properties', scrollable=True, resizable=True,
