...
class EnSight6BinaryReader(EnSightReader):
    """
    EnSight6BinaryReader - class to read binary en_sight6 files
    
    Superclass: EnSightReader
    
    EnSight6BinaryReader is a class to read binary en_sight6 files into
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkEnSight6BinaryReader, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(EnSight6BinaryReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit EnSight6BinaryReader properties', scrollable=True, resizable=True,
