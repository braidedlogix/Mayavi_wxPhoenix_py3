...
class XMLPDataSetWriter(XMLPDataWriter):
    """
    XMLPDataSetWriter - Write any type of PVTK XML file.
    
    Superclass: XMLPDataWriter
    
    XMLPDataSetWriter is a wrapper around the PVTK XML file format
    writers.  Given an input DataSet, the correct writer is
    automatically selected based on the type of input.
    
    @sa
    XMLPImageDataWriter XMLPStructuredGridWriter
    XMLPRectilinearGridWriter XMLPPolyDataWriter
    XMLPUnstructuredGridWriter
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkXMLPDataSetWriter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(XMLPDataSetWriter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit XMLPDataSetWriter properties', scrollable=True, resizable=True,
