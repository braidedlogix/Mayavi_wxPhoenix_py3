...
class UniformGridAMRAlgorithm(Algorithm):
    """
    UniformGridAMRAlgorithm -  UniformGridAMR as output.
    
    Superclass: Algorithm
    
    A base class for all algorithms that take as input any type of data
    object
     including composite datasets and produce UniformGridAMR in the
    output.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkUniformGridAMRAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        """
        V.get_output() -> UniformGridAMR
        C++: UniformGridAMR *GetOutput()
        V.get_output(int) -> UniformGridAMR
        C++: UniformGridAMR *GetOutput(int)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(UniformGridAMRAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit UniformGridAMRAlgorithm properties', scrollable=True, resizable=True,
