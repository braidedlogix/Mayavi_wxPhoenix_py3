...
class StructuredGridPartitioner(MultiBlockDataSetAlgorithm):
    """
    StructuredGridPartitioner -  A concrete implementation of
    ...
    @sa
     UniformGridPartitioner RectilinearGridPartitioner
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkStructuredGridPartitioner, obj, update, **traits)
    
    duplicate_nodes = tvtk_base.true_bool_trait(help=\
        """
        Set/Get & boolean macro for the duplicate_nodes property.
        """
    )
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(StructuredGridPartitioner, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit StructuredGridPartitioner properties', scrollable=True, resizable=True,
