...
class ImageMagnitude(ThreadedImageAlgorithm):
    """
    ImageMagnitude - Colapses components with magnitude function..
    
    Superclass: ThreadedImageAlgorithm
    
    ImageMagnitude takes the magnitude of the components.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageMagnitude, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageMagnitude, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageMagnitude properties', scrollable=True, resizable=True,
