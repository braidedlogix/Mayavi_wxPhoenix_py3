...
class HyperOctreeCutter(PolyDataAlgorithm):
    """
    HyperOctreeCutter - Cut HyperOctree with user-specified
    implicit function
    
    Superclass: PolyDataAlgorithm
    
    HyperOctreeCutter is a filter to cut through data using any
    subclass of ImplicitFunction. That is, a polygonal surface is
    ...
    HyperOctreeCutter is generally used to "slice-through" a dataset,
    generating a surface that can be visualized. It is also possible to
    use HyperOctreeCutter to do a form of volume rendering.
    HyperOctreeCutter does this by generating multiple cut surfaces
    ...
    @sa
    ImplicitFunction HyperOctree
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkHyperOctreeCutter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(HyperOctreeCutter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit HyperOctreeCutter properties', scrollable=True, resizable=True,
            handler=TVTKBaseHandler,
            buttons=['OK', 'Cancel'])
            return full_traits_view
        elif name == 'view':
            view = \
            View((['generate_cut_scalars'], ['sort_by'], ['number_of_contours']),
            title='Edit HyperOctreeCutter properties', scrollable=True, resizable=True,
            handler=TVTKBaseHandler,
            buttons=['OK', 'Cancel'])
            return view
