...
class MatrixToLinearTransform(LinearTransform):
    """
    MatrixToLinearTransform - convert a matrix to a transform
    
    Superclass: LinearTransform
    
    This is a very simple class which allows a Matrix4x4 to be used in
    place of a LinearTransform or AbstractTransform.  For example,
    ...
    @sa
    Transform Matrix4x4 MatrixToHomogeneousTransform
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkMatrixToLinearTransform, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(MatrixToLinearTransform, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit MatrixToLinearTransform properties', scrollable=True, resizable=True,
