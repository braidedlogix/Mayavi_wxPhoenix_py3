...
class Community2DLayoutStrategy(GraphLayoutStrategy):
    """
    Community2DLayoutStrategy - a simple fast 2d graph layout that
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkCommunity2DLayoutStrategy, obj, update, **traits)
    
    community_array_name = traits.String('community', enter_set=True, auto_set=False, help=\
        """
        Get/Set the community array name
        """
    )

    def _community_array_name_changed(self, old_val, new_val):
        self._do_change(self._vtk_obj.SetCommunityArrayName,
                        self.community_array_name)

    community_strength = traits.Trait(1.0, traits.Range(0.10000000149011612, 1.0, enter_set=True, auto_set=False), help=\
        """
    ...
    def _community_strength_changed(self, old_val, new_val):
        self._do_change(self._vtk_obj.SetCommunityStrength,
    ...
    'GetGlobalWarningDisplay'), ('community_array_name',
    'GetCommunityArrayName'), ('community_strength',
    'GetCommunityStrength'), ('cool_down_rate', 'GetCoolDownRate'),
    ...
    ('random_seed', 'GetRandomSeed'), ('rest_distance',
    'GetRestDistance'), ('edge_weight_field', 'GetEdgeWeightField'),
    ('weight_edges', 'GetWeightEdges'), ('reference_count',
    'GetReferenceCount'))
    ...
    (['debug', 'global_warning_display', 'community_array_name',
    'community_strength', 'cool_down_rate', 'edge_weight_field',
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(Community2DLayoutStrategy, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit Community2DLayoutStrategy properties', scrollable=True, resizable=True,
            handler=TVTKBaseHandler,
            buttons=['OK', 'Cancel'])
            return full_traits_view
        elif name == 'view':
            view = \
            View(([], [], ['community_array_name', 'community_strength',
            'cool_down_rate', 'edge_weight_field', 'initial_temperature',
