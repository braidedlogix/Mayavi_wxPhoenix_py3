...
class BrokenLineWidget(ThreeDWidget):
    """
    BrokenLineWidget - 3d widget for manipulating a broken line
    ...
    BrokenLineWidget, like any 3d widget, will work with the current
    interactor style. That is, if BrokenLineWidget does not handle an
    ...
    BrokenLineWidget will terminate the processing of the event that
    it handles.
    ...
    which point the BrokenLineWidget will appear. (See superclass
    ...
    The BrokenLineWidget has several methods that can be used in
    ...
    the widget is to make use of the start_interaction_event,
    interaction_event, and end_interaction_event events. The
    interaction_event is called on mouse motion; the other two events are
    ...
    calling the get_summed_length method!
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkBrokenLineWidget, obj, update, **traits)
    
    def setup_observers(self):
        """Setup the observers for the object."""
        super(BrokenLineWidget, self).setup_observers()
    ...
    project_to_plane = tvtk_base.false_bool_trait(help=\
        ...
        modified_event is invoked. This can be used to snap the broken
    ...
    handle_size_factor = traits.Trait(1.0, traits.Range(0.0, 100.0, enter_set=True, auto_set=False), help=\
        """
    ...
    def _handle_size_factor_changed(self, old_val, new_val):
        self._do_change(self._vtk_obj.SetHandleSizeFactor,
                        self.handle_size_factor)
    ...
    def get_poly_data(self, *args):
        ...
        numbering n_handles and n_handles - 1, respectively. Points are
    ...
    _updateable_traits_ = \
    (('process_events', 'GetProcessEvents'), ('project_to_plane',
    'GetProjectToPlane'), ('enabled', 'GetEnabled'),
    ('key_press_activation', 'GetKeyPressActivation'), ('picking_managed',
    'GetPickingManaged'), ('debug', 'GetDebug'),
    ('global_warning_display', 'GetGlobalWarningDisplay'),
    ('projection_normal', 'GetProjectionNormal'), ('handle_size_factor',
    'GetHandleSizeFactor'), ('number_of_handles', 'GetNumberOfHandles'),
    ('projection_position', 'GetProjectionPosition'), ('handle_size',
    ...
    'key_press_activation', 'picking_managed', 'process_events',
    'project_to_plane', 'projection_normal', 'handle_size',
    'handle_size_factor', 'key_press_activation_value',
    'number_of_handles', 'place_factor', 'priority',
    'projection_position'])
    
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(BrokenLineWidget, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit BrokenLineWidget properties', scrollable=True, resizable=True,
            ...
            ['handle_size', 'handle_size_factor', 'key_press_activation_value',
            'number_of_handles', 'place_factor', 'priority',
            'projection_position']),
            title='Edit BrokenLineWidget properties', scrollable=True, resizable=True,
            handler=TVTKBaseHandler,
