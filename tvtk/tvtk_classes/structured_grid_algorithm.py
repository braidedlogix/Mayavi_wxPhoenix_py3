...
class StructuredGridAlgorithm(Algorithm):
    """
    StructuredGridAlgorithm - Superclass for algorithms that produce
    only structured grid as output
    
    Superclass: Algorithm
    
    StructuredGridAlgorithm is a convenience class to make writing
    ...
    default says that all inputs will be structured_grid. If that isn't
    the case then please override this method in your subclass.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkStructuredGridAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        ...
        C++: StructuredGrid *GetOutput(int)
    ...
    def get_structured_grid_input(self, *args):
        """
        V.get_structured_grid_input(int) -> StructuredGrid
        C++: StructuredGrid *GetStructuredGridInput(int port)"""
        ret = self._wrap_call(self._vtk_obj.GetStructuredGridInput, *args)
        return wrap_vtk(ret)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(StructuredGridAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit StructuredGridAlgorithm properties', scrollable=True, resizable=True,
