...
from tvtk.tvtk_classes.cutter import Cutter


class CompositeCutter(Cutter):
    """
    CompositeCutter - Cut composite data sets with user-specified
    implicit function
    
    Superclass: Cutter
    
    Loop over each data set in the composite input and apply Cutter
    @sa
    Cutter
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkCompositeCutter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(CompositeCutter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit CompositeCutter properties', scrollable=True, resizable=True,
