...
class ImageGradient(ThreadedImageAlgorithm):
    """
    ImageGradient - Computes the gradient vector.
    
    Superclass: ThreadedImageAlgorithm
    
    ImageGradient computes the gradient vector of an image.  The
    ...
    dimensional XY gradient.  output_scalar_type is always double. Gradient
    is computed using central differences.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageGradient, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageGradient, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageGradient properties', scrollable=True, resizable=True,
