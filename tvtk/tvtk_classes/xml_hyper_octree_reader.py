...
class XMLHyperOctreeReader(XMLDataReader):
    """
    XMLHyperOctreeReader - Read VTK XML hyper_octree files.
    
    Superclass: XMLDataReader
    
    XMLHyperOctreeReader reads the VTK XML hyper_octree file format. 
    ...
    @sa
    XMLPHyperOctreeReader
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkXMLHyperOctreeReader, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        ...
        C++: HyperOctree *GetOutput(int idx)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(XMLHyperOctreeReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit XMLHyperOctreeReader properties', scrollable=True, resizable=True,
