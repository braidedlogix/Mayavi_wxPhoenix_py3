...
class XMLDataElement(Object):
    ...
    character_data = traits.String('', enter_set=True, auto_set=False, help=\
        """
    ...
    character_data_width = traits.Int(-1, enter_set=True, auto_set=False, help=\
        ...
        insterting line feeds every width number of fields. See print_xml.
        """
    )
    ...
    def get_vector_attribute(self, *args):
        """
        V.get_vector_attribute(string, int, [int, ...]) -> int
        C++: int GetVectorAttribute(const char *name, int length,
            int *value)
        V.get_vector_attribute(string, int, [float, ...]) -> int
        C++: int GetVectorAttribute(const char *name, int length,
            double *value)
        V.get_vector_attribute(string, int, [int, ...]) -> int
        C++: int GetVectorAttribute(const char *name, int length,
    ...
    def set_vector_attribute(self, *args):
        """
        V.set_vector_attribute(string, int, (int, ...))
        C++: void SetVectorAttribute(const char *name, int length,
            const int *value)
        V.set_vector_attribute(string, int, (float, ...))
        C++: void SetVectorAttribute(const char *name, int length,
            const double *value)
        V.set_vector_attribute(string, int, (int, ...))
        C++: void SetVectorAttribute(const char *name, int length,
    ...
    def trait_view(self, name=None, view_element=None):
        ...
        if name == 'full_traits_view':
            ...
            'character_data_width', 'id', 'name', 'xml_byte_index']),
            title='Edit XMLDataElement properties', scrollable=True, resizable=True,
            handler=TVTKBaseHandler,
            buttons=['OK', 'Cancel'])
            return view
