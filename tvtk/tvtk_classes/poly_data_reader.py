...
class PolyDataReader(DataReader):
    """
    PolyDataReader - read vtk polygonal data file
    
    Superclass: DataReader
    
    PolyDataReader is a source object that reads ASCII or binary
    polygonal data files in vtk format (see text for format details). The
    output of this reader is a single PolyData data object. The
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkPolyDataReader, obj, update, **traits)
    ...
    def set_output(self, obj):
        """
        V.set_output(PolyData)
        C++: void SetOutput(PolyData *output)
        Get the output of this reader.
        """
        old_val = self._get_output()
        self._wrap_call(self._vtk_obj.SetOutput, deref_vtk(obj))
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(PolyDataReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit PolyDataReader properties', scrollable=True, resizable=True,
