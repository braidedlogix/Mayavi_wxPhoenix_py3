...
class GeoArcs(PolyDataAlgorithm):
    """
    GeoArcs - Layout graph edges on a globe as arcs.
    
    Superclass: PolyDataAlgorithm
    
    GeoArcs produces arcs for each line in the input polydata. This is
    useful for viewing lines on a sphere (e.g. the earth). The arcs may
    "jump" above the sphere's surface using explode_factor.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkGeoArcs, obj, update, **traits)
    ...
    ('explode_factor', 'GetExplodeFactor'), ('globe_radius',
    'GetGlobeRadius'), ('number_of_subdivisions',
    'GetNumberOfSubdivisions'), ('progress', 'GetProgress'),
    ...
    'release_data_flag', 'explode_factor', 'globe_radius',
    'number_of_subdivisions', 'progress_text'])
    
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(GeoArcs, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit GeoArcs properties', scrollable=True, resizable=True,
            handler=TVTKBaseHandler,
            buttons=['OK', 'Cancel'])
            return full_traits_view
        elif name == 'view':
            view = \
            View(([], [], ['explode_factor', 'globe_radius',
