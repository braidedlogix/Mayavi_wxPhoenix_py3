...
class ImageButterworthHighPass(ThreadedImageAlgorithm):
    """
    ImageButterworthHighPass - Frequency domain high pass.
    ...
    ImageButterworthHighPass  the frequency components around 0 are
    attenuated.  Input and output are in doubles, with two components
    (complex numbers). out(i, j) = 1 / (1 + pow(_cut_off/_freq(i,j),
    2*Order));
    
    @sa
    ImageButterworthLowPass
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageButterworthHighPass, obj, update, **traits)
    ...
    'GetSplitMode'), ('cut_off', 'GetCutOff'), ('order', 'GetOrder'),
    ('x_cut_off', 'GetXCutOff'), ('y_cut_off', 'GetYCutOff'),
    ('z_cut_off', 'GetZCutOff'), ('desired_bytes_per_piece',
    ...
    'minimum_piece_size', 'number_of_threads', 'order', 'progress_text',
    'x_cut_off', 'y_cut_off', 'z_cut_off'])
    
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageButterworthHighPass, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageButterworthHighPass properties', scrollable=True, resizable=True,
            ...
            'number_of_threads', 'order', 'x_cut_off', 'y_cut_off', 'z_cut_off']),
            title='Edit ImageButterworthHighPass properties', scrollable=True, resizable=True,
            handler=TVTKBaseHandler,
            buttons=['OK', 'Cancel'])
            return view
