...
class PlotGrid(ContextItem):
    """
    PlotGrid - takes care of drawing the plot grid
    
    Superclass: ContextItem
    
    The PlotGrid is drawn in screen coordinates. It is usually one of
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkPlotGrid, obj, update, **traits)
    
    def set_x_axis(self, *args):
        """
        V.set_x_axis(Axis)
        C++: virtual void SetXAxis(Axis *axis)
        Set the X axis of the grid.
        """
        my_args = [deref_vtk(x) for x in args]
        ret = self._wrap_call(self._vtk_obj.SetXAxis, *my_args)
        return ret

    def set_y_axis(self, *args):
        """
        V.set_y_axis(Axis)
        C++: virtual void SetYAxis(Axis *axis)
        Set the X axis of the grid.
        """
        my_args = [deref_vtk(x) for x in args]
        ret = self._wrap_call(self._vtk_obj.SetYAxis, *my_args)
        return ret
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(PlotGrid, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit PlotGrid properties', scrollable=True, resizable=True,
