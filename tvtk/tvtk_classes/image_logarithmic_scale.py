...
class ImageLogarithmicScale(ThreadedImageAlgorithm):
    """
    ImageLogarithmicScale - Passes each pixel through log function.
    
    Superclass: ThreadedImageAlgorithm
    
    ImageLogarithmicScale passes each pixel through the function
    c*log(1+x).  It also handles negative values with the function
    -c*log(1-x).
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageLogarithmicScale, obj, update, **traits)
    
    constant = traits.Float(10.0, enter_set=True, auto_set=False, help=\
        """
    ...
    'GetSplitMode'), ('constant', 'GetConstant'),
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageLogarithmicScale, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageLogarithmicScale properties', scrollable=True, resizable=True,
