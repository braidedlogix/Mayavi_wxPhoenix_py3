...
class XMLPPolyDataReader(XMLPUnstructuredDataReader):
    """
    XMLPPolyDataReader - Read PVTK XML poly_data files.
    
    Superclass: XMLPUnstructuredDataReader
    
    XMLPPolyDataReader reads the PVTK XML poly_data file format. This
    reads the parallel format's summary file and then uses
    XMLPolyDataReader to read data from the individual poly_data piece
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkXMLPPolyDataReader, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(XMLPPolyDataReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit XMLPPolyDataReader properties', scrollable=True, resizable=True,
