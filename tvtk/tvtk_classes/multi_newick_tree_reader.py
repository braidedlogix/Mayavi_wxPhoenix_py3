...
class MultiNewickTreeReader(DataReader):
    """
    MultiNewickTreeReader - read multiple Trees from Newick
    formatted file
    
    Superclass: DataReader
    
    MultiNewickTreeReader is a source object that reads Newick tree
    format files. The output of this reader is a single
    MultiPieceDataSet that contains multiple Tree objects. The
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkMultiNewickTreeReader, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        """
        V.get_output() -> MultiPieceDataSet
        C++: MultiPieceDataSet *GetOutput()
        V.get_output(int) -> MultiPieceDataSet
        C++: MultiPieceDataSet *GetOutput(int idx)
    ...
    def set_output(self, obj):
        """
        V.set_output(MultiPieceDataSet)
        C++: void SetOutput(MultiPieceDataSet *output)
        Get the output of this reader.
        """
        old_val = self._get_output()
        self._wrap_call(self._vtk_obj.SetOutput, deref_vtk(obj))
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(MultiNewickTreeReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit MultiNewickTreeReader properties', scrollable=True, resizable=True,
