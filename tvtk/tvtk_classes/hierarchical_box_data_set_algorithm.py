...
class HierarchicalBoxDataSetAlgorithm(Algorithm):
    """
    HierarchicalBoxDataSetAlgorithm - superclass for algorithms that
    produce HierarchicalBoxDataSet as output.
    
    Superclass: Algorithm
    
    Algorithms that take any type of data object (including composite
    dataset) and produce a HierarchicalBoxDataSet in the output can
    subclass from this class.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkHierarchicalBoxDataSetAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        """
        V.get_output() -> HierarchicalBoxDataSet
        C++: HierarchicalBoxDataSet *GetOutput()
        V.get_output(int) -> HierarchicalBoxDataSet
        C++: HierarchicalBoxDataSet *GetOutput(int)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(HierarchicalBoxDataSetAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit HierarchicalBoxDataSetAlgorithm properties', scrollable=True, resizable=True,
