...
class GraphAlgorithm(Algorithm):
    """
    GraphAlgorithm - Superclass for algorithms that produce only graph
    as output
    
    Superclass: Algorithm
    
    GraphAlgorithm is a convenience class to make writing algorithms
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkGraphAlgorithm, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(GraphAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit GraphAlgorithm properties', scrollable=True, resizable=True,
