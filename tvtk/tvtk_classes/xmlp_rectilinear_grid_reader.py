...
class XMLPRectilinearGridReader(XMLPStructuredDataReader):
    """
    XMLPRectilinearGridReader - Read PVTK XML rectilinear_grid files.
    
    Superclass: XMLPStructuredDataReader
    
    XMLPRectilinearGridReader reads the PVTK XML rectilinear_grid file
    format.  This reads the parallel format's summary file and then uses
    XMLRectilinearGridReader to read data from the individual
    rectilinear_grid piece files.  Streaming is supported. The standard
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkXMLPRectilinearGridReader, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(XMLPRectilinearGridReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit XMLPRectilinearGridReader properties', scrollable=True, resizable=True,
