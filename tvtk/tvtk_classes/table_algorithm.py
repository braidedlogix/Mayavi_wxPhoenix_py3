...
class TableAlgorithm(Algorithm):
    """
    TableAlgorithm - Superclass for algorithms that produce only
    Tables as output
    
    Superclass: Algorithm
    
    TableAlgorithm is a convenience class to make writing algorithms
    ...
    a fill_input_port_info method that by default says that all inputs will
    be Tree. If that isn't the case then please override this method in
    your subclass.
    
    @par Thanks: Thanks to Brian Wylie for creating this class.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkTableAlgorithm, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(TableAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit TableAlgorithm properties', scrollable=True, resizable=True,
