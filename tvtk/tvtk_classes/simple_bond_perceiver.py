...
class SimpleBondPerceiver(MoleculeAlgorithm):
    """
    SimpleBondPerceiver - Create a simple guess of a molecule's
    topology
    
    Superclass: MoleculeAlgorithm
    
    SimpleBondPerceiver performs a simple check of all interatomic
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkSimpleBondPerceiver, obj, update, **traits)
    
    tolerance = traits.Float(0.44999998807907104, enter_set=True, auto_set=False, help=\
        """
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(SimpleBondPerceiver, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit SimpleBondPerceiver properties', scrollable=True, resizable=True,
