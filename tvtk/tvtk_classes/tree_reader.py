...
class TreeReader(DataReader):
    """
    TreeReader - read Tree data file
    
    Superclass: DataReader
    
    TreeReader is a source object that reads ASCII or binary Tree
    data files in vtk format. (see text for format details). The output
    of this reader is a single Tree data object. The superclass of
    ...
    systems.
    @sa
    Tree DataReader TreeWriter
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkTreeReader, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(TreeReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit TreeReader properties', scrollable=True, resizable=True,
