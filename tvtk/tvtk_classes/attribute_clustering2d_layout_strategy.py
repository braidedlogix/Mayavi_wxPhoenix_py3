...
class AttributeClustering2DLayoutStrategy(GraphLayoutStrategy):
    """
    AttributeClustering2DLayoutStrategy - a simple fast 2d graph layout
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkAttributeClustering2DLayoutStrategy, obj, update, **traits)
    ...
    vertex_attribute = traits.Trait(None, None, traits.String(enter_set=True, auto_set=False), help=\
        """
    ...
    def _vertex_attribute_changed(self, old_val, new_val):
        self._do_change(self._vtk_obj.SetVertexAttribute,
    ...
    'GetRestDistance'), ('vertex_attribute', 'GetVertexAttribute'),
    ...
    'vertex_attribute', 'weight_edges'])
    
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(AttributeClustering2DLayoutStrategy, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit AttributeClustering2DLayoutStrategy properties', scrollable=True, resizable=True,
