...
class FacetWriter(PolyDataAlgorithm):
    """
    FacetWriter - reads a dataset in Facet format
    
    Superclass: PolyDataAlgorithm
    
    FacetWriter creates an unstructured grid dataset. It reads ASCII
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkFacetWriter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(FacetWriter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit FacetWriter properties', scrollable=True, resizable=True,
