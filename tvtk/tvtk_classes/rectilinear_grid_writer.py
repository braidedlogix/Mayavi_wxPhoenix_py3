...
class RectilinearGridWriter(DataWriter):
    """
    RectilinearGridWriter - write vtk rectilinear grid data file
    
    Superclass: DataWriter
    
    RectilinearGridWriter is a source object that writes ASCII or
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkRectilinearGridWriter, obj, update, **traits)
    ...
    def get_input(self, *args):
        ...
        V.get_input(int) -> RectilinearGrid
        C++: RectilinearGrid *GetInput(int port)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(RectilinearGridWriter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit RectilinearGridWriter properties', scrollable=True, resizable=True,
