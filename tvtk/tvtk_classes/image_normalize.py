...
class ImageNormalize(ThreadedImageAlgorithm):
    """
    ImageNormalize - Normalizes that scalar components for each point.
    
    Superclass: ThreadedImageAlgorithm
    
    For each point, ImageNormalize normalizes the vector defined by
    the scalar components.  If the magnitude of this vector is zero, the
    output vector is zero also.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageNormalize, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageNormalize, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageNormalize properties', scrollable=True, resizable=True,
