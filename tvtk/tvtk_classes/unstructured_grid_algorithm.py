...
class UnstructuredGridAlgorithm(Algorithm):
    """
    UnstructuredGridAlgorithm - Superclass for algorithms that produce
    only unstructured grid as output
    
    Superclass: Algorithm
    
    UnstructuredGridAlgorithm is a convenience class to make writing
    ...
    default says that all inputs will be unstructured_grid. If that isn't
    the case then please override this method in your subclass.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkUnstructuredGridAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        ...
        C++: UnstructuredGrid *GetOutput(int)
    ...
    def get_unstructured_grid_input(self, *args):
        """
        V.get_unstructured_grid_input(int) -> UnstructuredGrid
        C++: UnstructuredGrid *GetUnstructuredGridInput(int port)"""
        ret = self._wrap_call(self._vtk_obj.GetUnstructuredGridInput, *args)
        return wrap_vtk(ret)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(UnstructuredGridAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit UnstructuredGridAlgorithm properties', scrollable=True, resizable=True,
