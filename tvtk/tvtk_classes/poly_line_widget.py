...
class PolyLineWidget(AbstractWidget):
    """
    PolyLineWidget - widget for PolyLineRepresentation.
    
    Superclass: AbstractWidget
    
    PolyLineWidget is the AbstractWidget subclass for
    PolyLineRepresentation which manages the interactions with
    PolyLineRepresentation. This is based on PolyLineWidget.
    @sa
    PolyLineRepresentation, PolyLineWidget
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkPolyLineWidget, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(PolyLineWidget, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit PolyLineWidget properties', scrollable=True, resizable=True,
