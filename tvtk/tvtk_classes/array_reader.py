...
class ArrayReader(ArrayDataAlgorithm):
    """
    ArrayReader - Reads sparse and dense Array data written by
    ArrayWriter.
    
    Superclass: ArrayDataAlgorithm
    
    Reads sparse and dense Array data written with ArrayWriter.
    
    Outputs:
      Output port 0: ArrayData containing a dense or sparse array.
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkArrayReader, obj, update, **traits)
    ...
    def read(self, *args):
        """
        V.read(string) -> Array
        C++: static Array *Read(StdString str)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ArrayReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ArrayReader properties', scrollable=True, resizable=True,
