...
class ArrayDataWriter(Writer):
    """
    ArrayDataWriter - Serialize ArrayData to a file or stream.
    
    Superclass: Writer
    
    ArrayDataWriter serializes ArrayData using a text-based format
    that is human-readable and easily parsed (default option).  The
    write_binary array option can be used to serialize the ArrayData
    using a binary format that is optimized for rapid throughput.
    
    ArrayDataWriter can be used in two distinct ways: first, it can be
    used as a normal pipeline filter, which writes its inputs to a file. 
    Alternatively, static methods are provided for writing ArrayData
    instances to files or arbitrary c++ streams.
    
    Inputs:
      Input port 0: (required) ArrayData object.
    
    Output Format:
      See
    http://www.kitware.com/_infovis_wiki/index.php/_n-_way__array__file__formats
    for
      details on how ArrayDataWriter encodes data.
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkArrayDataWriter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ArrayDataWriter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ArrayDataWriter properties', scrollable=True, resizable=True,
