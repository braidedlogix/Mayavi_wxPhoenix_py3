...
from tvtk.tvtk_classes.molecule_algorithm import MoleculeAlgorithm


class GaussianCubeReader2(MoleculeAlgorithm):
    """
    GaussianCubeReader2 - Read a Gaussian Cube file and output a
    Molecule object and a ImageData
    
    Superclass: MoleculeAlgorithm
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkGaussianCubeReader2, obj, update, **traits)
    ...
    def get_output(self):
        ...
        Get/Set the output (vtk_molecule) that the reader will fill
        """
        return wrap_vtk(self._vtk_obj.GetOutput())
    ...
    grid_output = traits.Property(_get_grid_output, help=\
        """
        Get/Set the output (vtk_image_data) that the reader will fill
        """
    )
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(GaussianCubeReader2, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit GaussianCubeReader2 properties', scrollable=True, resizable=True,
