...
class ImageVariance3D(ImageSpatialAlgorithm):
    """
    ImageVariance3D - Variance in a neighborhood.
    
    Superclass: ImageSpatialAlgorithm
    
    ImageVariance3D replaces each pixel with a measurement of pixel
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageVariance3D, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageVariance3D, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageVariance3D properties', scrollable=True, resizable=True,
