...
class RectilinearGridClip(RectilinearGridAlgorithm):
    """
    RectilinearGridClip - Reduces the image extent of the input.
    
    Superclass: RectilinearGridAlgorithm
    
    RectilinearGridClip  will make an image smaller.  The output must
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkRectilinearGridClip, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(RectilinearGridClip, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit RectilinearGridClip properties', scrollable=True, resizable=True,
