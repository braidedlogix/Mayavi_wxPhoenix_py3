...
class VolumeContourSpectrumFilter(DataObjectAlgorithm):
    """
    VolumeContourSpectrumFilter - compute an approximation of the
    ...
    with a ReebGraph (port 1). The Reeb graph arc to consider can be
    ...
    UnstructuredGrid, see set_field_id (default: 0). The filter will
    abort if this field does not exist.
    
    The filter outputs a Table with the volume contour signature
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkVolumeContourSpectrumFilter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(VolumeContourSpectrumFilter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit VolumeContourSpectrumFilter properties', scrollable=True, resizable=True,
