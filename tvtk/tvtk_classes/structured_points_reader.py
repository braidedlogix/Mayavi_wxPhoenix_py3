...
class StructuredPointsReader(DataReader):
    """
    StructuredPointsReader - read vtk structured points data file
    
    Superclass: DataReader
    
    StructuredPointsReader is a source object that reads ASCII or
    binary structured points data files in vtk format (see text for
    format details). The output of this reader is a single
    StructuredPoints data object. The superclass of this class,
    ...
    systems.
    @sa
    StructuredPoints DataReader
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkStructuredPointsReader, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        ...
        C++: StructuredPoints *GetOutput(int idx)
        V.get_output() -> StructuredPoints
        C++: StructuredPoints *GetOutput()
    ...
    def set_output(self, obj):
        """
        V.set_output(StructuredPoints)
        C++: void SetOutput(StructuredPoints *output)
        Set/Get the output of this reader.
        """
        old_val = self._get_output()
        self._wrap_call(self._vtk_obj.SetOutput, deref_vtk(obj))
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(StructuredPointsReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit StructuredPointsReader properties', scrollable=True, resizable=True,
