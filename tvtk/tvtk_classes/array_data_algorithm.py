...
class ArrayDataAlgorithm(Algorithm):
    """
    ArrayDataAlgorithm - Superclass for algorithms that produce
    ArrayDatas as output
    
    Superclass: Algorithm
    
    ArrayDataAlgorithm is a convenience class to make writing
    ...
    default says that all inputs will be ArrayData. If that isn't the
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkArrayDataAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        """
        V.get_output() -> ArrayData
        C++: ArrayData *GetOutput()
        V.get_output(int) -> ArrayData
        C++: ArrayData *GetOutput(int index)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ArrayDataAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ArrayDataAlgorithm properties', scrollable=True, resizable=True,
