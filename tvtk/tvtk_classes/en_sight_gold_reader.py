...
class EnSightGoldReader(EnSightReader):
    """
    EnSightGoldReader - class to read en_sight Gold files
    
    Superclass: EnSightReader
    
    EnSightGoldReader is a class to read en_sight Gold files into vtk.
    Because the different parts of the en_sight data can be of various
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkEnSightGoldReader, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(EnSightGoldReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit EnSightGoldReader properties', scrollable=True, resizable=True,
