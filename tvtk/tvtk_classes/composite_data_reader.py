...
class CompositeDataReader(DataReader):
    """
    CompositeDataReader - read CompositeDataSet data file.
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkCompositeDataReader, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        ...
        C++: CompositeDataSet *GetOutput(int idx)
    ...
    def set_output(self, obj):
        """
        V.set_output(CompositeDataSet)
        C++: void SetOutput(CompositeDataSet *output)
        Get the output of this reader.
        """
        old_val = self._get_output()
        self._wrap_call(self._vtk_obj.SetOutput, deref_vtk(obj))
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(CompositeDataReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit CompositeDataReader properties', scrollable=True, resizable=True,
