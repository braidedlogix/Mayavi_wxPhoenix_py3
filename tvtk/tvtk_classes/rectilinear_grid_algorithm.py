...
class RectilinearGridAlgorithm(Algorithm):
    """
    RectilinearGridAlgorithm - Superclass for algorithms that produce
    only rectilinear grid as output
    
    Superclass: Algorithm
    
    RectilinearGridAlgorithm is a convenience class to make writing
    ...
    default says that all inputs will be rectilinear_grid. If that isn't
    the case then please override this method in your subclass. You
    should implement the subclass's algorithm into request_data( request,
    input_vec, output_vec).
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkRectilinearGridAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        ...
        C++: RectilinearGrid *GetOutput(int)
    ...
    def get_rectilinear_grid_input(self, *args):
        """
        V.get_rectilinear_grid_input(int) -> RectilinearGrid
        C++: RectilinearGrid *GetRectilinearGridInput(int port)"""
        ret = self._wrap_call(self._vtk_obj.GetRectilinearGridInput, *args)
        return wrap_vtk(ret)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(RectilinearGridAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit RectilinearGridAlgorithm properties', scrollable=True, resizable=True,
