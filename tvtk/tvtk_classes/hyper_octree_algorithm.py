...
class HyperOctreeAlgorithm(Algorithm):
    """
    HyperOctreeAlgorithm - Superclass for algorithms that produce only
    octree as output
    
    Superclass: Algorithm
    
    OctreeAlgorithm is a convenience class to make writing algorithms
    ...
    inputs will be hyper_octree. If that isn't the case then please
    override this method in your subclass.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkHyperOctreeAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        """
        V.get_output() -> HyperOctree
        C++: HyperOctree *GetOutput()
        V.get_output(int) -> HyperOctree
        C++: HyperOctree *GetOutput(int)
    ...
    def get_hyper_octree_input(self, *args):
        """
        V.get_hyper_octree_input(int) -> HyperOctree
        C++: HyperOctree *GetHyperOctreeInput(int port)"""
        ret = self._wrap_call(self._vtk_obj.GetHyperOctreeInput, *args)
        return wrap_vtk(ret)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(HyperOctreeAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit HyperOctreeAlgorithm properties', scrollable=True, resizable=True,
