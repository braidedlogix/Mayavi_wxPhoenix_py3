...
class XMLDataSetWriter(XMLWriter):
    ...
    def new_writer(self, *args):
        """
        V.new_writer(int) -> XMLWriter
        C++: static XMLWriter *NewWriter(int dataset_type)
        Creates a writer for the given dataset type. May return NULL for
        unsupported/unrecognized dataset types. Returns a new instance.
        The caller is responsible of calling Object::Delete() or
        Object::UnRegister() on it when done.
        """
        ret = self._wrap_call(self._vtk_obj.NewWriter, *args)
        return wrap_vtk(ret)
