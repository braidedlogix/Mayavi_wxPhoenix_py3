...
from tvtk.tvtk_classes.transform import Transform


class SMPTransform(Transform):
    """
    SMPTransform - Transform that uses the SMP framework
    
    Superclass: Transform
    
    Just like its parent, Transform, SMPTransform calculates and
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkSMPTransform, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(SMPTransform, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit SMPTransform properties', scrollable=True, resizable=True,
