...
class PolyDataAlgorithm(Algorithm):
    """
    PolyDataAlgorithm - Superclass for algorithms that produce only
    polydata as output
    
    Superclass: Algorithm
    
    PolyDataAlgorithm is a convenience class to make writing
    ...
    default says that all inputs will be poly_data. If that isn't the case
    then please override this method in your subclass.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkPolyDataAlgorithm, obj, update, **traits)
    ...
    def get_poly_data_input(self, *args):
        """
        V.get_poly_data_input(int) -> PolyData
        C++: PolyData *GetPolyDataInput(int port)"""
        ret = self._wrap_call(self._vtk_obj.GetPolyDataInput, *args)
        return wrap_vtk(ret)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(PolyDataAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit PolyDataAlgorithm properties', scrollable=True, resizable=True,
