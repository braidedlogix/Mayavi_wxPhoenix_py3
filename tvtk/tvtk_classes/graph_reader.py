...
class GraphReader(DataReader):
    """
    GraphReader - read Graph data file
    
    Superclass: DataReader
    
    GraphReader is a source object that reads ASCII or binary Graph
    data files in vtk format. (see text for format details). The output
    of this reader is a single Graph data object. The superclass of
    ...
    systems.
    @sa
    Graph DataReader GraphWriter
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkGraphReader, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        ...
        V.get_output() -> Graph
        C++: Graph *GetOutput()
        V.get_output(int) -> Graph
        C++: Graph *GetOutput(int idx)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(GraphReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit GraphReader properties', scrollable=True, resizable=True,
