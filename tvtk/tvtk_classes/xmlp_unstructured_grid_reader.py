...
from tvtk.tvtk_classes.xmlp_unstructured_data_reader import XMLPUnstructuredDataReader


class XMLPUnstructuredGridReader(XMLPUnstructuredDataReader):
    """
    XMLPUnstructuredGridReader - Read PVTK XML unstructured_grid files.
    
    Superclass: XMLPUnstructuredDataReader
    
    XMLPUnstructuredGridReader reads the PVTK XML unstructured_grid
    file format.  This reads the parallel format's summary file and then
    uses XMLUnstructuredGridReader to read data from the individual
    unstructured_grid piece files.  Streaming is supported. The standard
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkXMLPUnstructuredGridReader, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(XMLPUnstructuredGridReader, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit XMLPUnstructuredGridReader properties', scrollable=True, resizable=True,
