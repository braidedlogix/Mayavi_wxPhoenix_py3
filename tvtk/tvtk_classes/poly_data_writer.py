...
class PolyDataWriter(DataWriter):
    """
    PolyDataWriter - write vtk polygonal data
    
    Superclass: DataWriter
    
    PolyDataWriter is a source object that writes ASCII or binary
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(PolyDataWriter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit PolyDataWriter properties', scrollable=True, resizable=True,
