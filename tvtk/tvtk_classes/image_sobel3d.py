...
class ImageSobel3D(ImageSpatialAlgorithm):
    """
    ImageSobel3D - Computes a vector field using sobel functions.
    
    Superclass: ImageSpatialAlgorithm
    
    ImageSobel3D computes a vector field from a scalar field by using
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageSobel3D, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageSobel3D, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageSobel3D properties', scrollable=True, resizable=True,
