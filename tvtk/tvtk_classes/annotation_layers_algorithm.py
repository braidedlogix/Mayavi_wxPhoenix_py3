...
class AnnotationLayersAlgorithm(Algorithm):
    """
    AnnotationLayersAlgorithm - Superclass for algorithms that produce
    only AnnotationLayers as output
    
    Superclass: Algorithm
    
    AnnotationLayersAlgorithm is a convenience class to make writing
    ...
    default says that all inputs will be AnnotationLayers. If that
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkAnnotationLayersAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        """
        V.get_output() -> AnnotationLayers
        C++: AnnotationLayers *GetOutput()
        V.get_output(int) -> AnnotationLayers
        C++: AnnotationLayers *GetOutput(int index)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(AnnotationLayersAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit AnnotationLayersAlgorithm properties', scrollable=True, resizable=True,
