...
class ClipHyperOctree(UnstructuredGridAlgorithm):
    """
    ClipHyperOctree - clip an hyperoctree with user-specified implicit
    function or input scalar data
    
    Superclass: UnstructuredGridAlgorithm
    
    ClipHyperOctree is a filter that clips an hyperoctree using either
    ...
    "pieces" of a cell. (Compare this with ExtractGeometry, which
    ...
    @warning
    ClipHyperOctree will triangulate all types of 3d cells (i.e.,
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkClipHyperOctree, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ClipHyperOctree, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ClipHyperOctree properties', scrollable=True, resizable=True,
