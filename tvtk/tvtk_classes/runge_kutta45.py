...
class RungeKutta45(InitialValueProblemSolver):
    """
    RungeKutta45 - Integrate an initial value problem using 5th order
    ...
    @sa
    InitialValueProblemSolver RungeKutta4 RungeKutta2
    FunctionSet
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkRungeKutta45, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(RungeKutta45, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit RungeKutta45 properties', scrollable=True, resizable=True,
