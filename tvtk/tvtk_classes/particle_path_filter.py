...
class ParticlePathFilter(ParticleTracerBase):
    """
    ParticlePathFilter - A Parallel Particle tracer for unsteady
    vector fields
    
    Superclass: ParticleTracerBase
    
    ParticlePathFilter is a filter that integrates a vector field to
    generate particle paths
    
    @sa
    ParticlePathFilterBase has the details of the algorithms
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkParticlePathFilter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ParticlePathFilter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ParticlePathFilter properties', scrollable=True, resizable=True,
