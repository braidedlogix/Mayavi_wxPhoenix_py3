...
class RectilinearGridGeometryFilter(PolyDataAlgorithm):
    """
    RectilinearGridGeometryFilter - extract geometry for a rectilinear
    grid
    
    Superclass: PolyDataAlgorithm
    
    RectilinearGridGeometryFilter is a filter that extracts geometry
    ...
    @sa
    GeometryFilter ExtractGrid
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkRectilinearGridGeometryFilter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(RectilinearGridGeometryFilter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit RectilinearGridGeometryFilter properties', scrollable=True, resizable=True,
