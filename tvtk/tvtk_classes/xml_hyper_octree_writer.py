...
class XMLHyperOctreeWriter(XMLWriter):
    """
    XMLHyperOctreeWriter - Write VTK XML hyper_octree files.
    
    Superclass: XMLWriter
    
    XMLHyperOctreeWriter writes the VTK XML hyper_octree file format. 
    One hyper_octree input can be written into one file in any number of
    ...
    @sa
    XMLPHyperOctreeWriter
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkXMLHyperOctreeWriter, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(XMLHyperOctreeWriter, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit XMLHyperOctreeWriter properties', scrollable=True, resizable=True,
