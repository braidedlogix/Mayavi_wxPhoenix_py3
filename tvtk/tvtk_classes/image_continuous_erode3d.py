...
class ImageContinuousErode3D(ImageSpatialAlgorithm):
    """
    ImageContinuousErode3D - Erosion implemented as a minimum.
    
    Superclass: ImageSpatialAlgorithm
    
    ImageContinuousErode3D replaces a pixel with the minimum over an
    ellipsoidal neighborhood.  If kernel_size of an axis is 1, no
    processing is done on that axis.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageContinuousErode3D, obj, update, **traits)
    ...
    'GetSplitMode'), ('kernel_size', 'GetKernelSize'),
    ...
    'enable_smp', 'global_default_enable_smp', 'kernel_size',
    'minimum_piece_size', 'number_of_threads', 'progress_text'])
    
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageContinuousErode3D, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageContinuousErode3D properties', scrollable=True, resizable=True,
