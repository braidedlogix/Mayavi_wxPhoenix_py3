...
class SelectionAlgorithm(Algorithm):
    """
    SelectionAlgorithm - Superclass for algorithms that produce only
    Selection as output
    
    Superclass: Algorithm
    
    SelectionAlgorithm is a convenience class to make writing
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkSelectionAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        """
        V.get_output() -> Selection
        C++: Selection *GetOutput()
        V.get_output(int) -> Selection
        C++: Selection *GetOutput(int index)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(SelectionAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit SelectionAlgorithm properties', scrollable=True, resizable=True,
