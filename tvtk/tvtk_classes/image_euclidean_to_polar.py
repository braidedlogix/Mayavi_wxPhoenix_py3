...
class ImageEuclideanToPolar(ThreadedImageAlgorithm):
    """
    ImageEuclideanToPolar - Converts 2d Euclidean coordinates to polar.
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageEuclideanToPolar, obj, update, **traits)
    
    theta_maximum = traits.Float(255.0, enter_set=True, auto_set=False, help=\
        """
        Theta is an angle. Maximum specifies when it maps back to 0.
        theta_maximum defaults to 255 instead of 2pi, because unsigned
        char is expected as input. The output type must be the same as
        input type.
        """
    )

    def _theta_maximum_changed(self, old_val, new_val):
        self._do_change(self._vtk_obj.SetThetaMaximum,
                        self.theta_maximum)
    ...
    'GetSplitMode'), ('theta_maximum', 'GetThetaMaximum'),
    ...
    'number_of_threads', 'progress_text', 'theta_maximum'])
    
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageEuclideanToPolar, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageEuclideanToPolar properties', scrollable=True, resizable=True,
            ...
            'number_of_threads', 'theta_maximum']),
            title='Edit ImageEuclideanToPolar properties', scrollable=True, resizable=True,
            handler=TVTKBaseHandler,
            buttons=['OK', 'Cancel'])
            return view
