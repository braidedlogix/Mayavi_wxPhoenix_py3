...
class PointSetAlgorithm(Algorithm):
    """
    PointSetAlgorithm - Superclass for algorithms that produce output
    of the same type as input
    
    Superclass: Algorithm
    
    PointSetAlgorithm is a convenience class to make writing
    ...
    default says that all inputs will be point_set. If that isn't the case
    then please override this method in your subclass. You should
    implement the subclass's algorithm into request_data( request,
    input_vec, output_vec).
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkPointSetAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        """
        V.get_output() -> PointSet
        C++: PointSet *GetOutput()
        V.get_output(int) -> PointSet
        C++: PointSet *GetOutput(int)
    ...
    def add_input_data(self, *args):
        ...
        V.add_input_data(PointSet)
        C++: void AddInputData(PointSet *)
        V.add_input_data(int, PointSet)
        C++: void AddInputData(int, PointSet *)
    ...
    def set_input_data(self, *args):
        ...
        V.set_input_data(PointSet)
        C++: void SetInputData(PointSet *)
        V.set_input_data(int, PointSet)
        C++: void SetInputData(int, PointSet *)
        Assign a data object as input. Note that this method does not
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(PointSetAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            View((Item("handler._full_traits_list",show_label=False)),
            title='Edit PointSetAlgorithm properties', scrollable=True, resizable=True,
