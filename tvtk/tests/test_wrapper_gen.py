...
class TestWrapperGenerator(unittest.TestCase):
    ...
    def test_find_type(self):
        wg = self.wg
        sigs = [
            'int', 'vtkOpenGLVolumeMapper', ('int', 'int', 'float', 'list'),
            ('int', 'vtkActor', 'vtkXMLReader'),
            ['vtkImageActor', 'vtkExporter'],
            ['int', 'vtkDataArray', 'vtkCellArray', 'vtkIdTypeArray']
        ]
    ...
    def test_sig_types(self):
        wg = self.wg
        meths = [
            vtk.vtkProperty.GetColor, vtk.vtkProperty.GetRepresentation,
            vtk.vtkStructuredPointsReader.GetOutput, vtk.vtkPolyData.SetPoints,
            vtk.vtkPolyData.SetPolys, vtk.vtkQuad.CellBoundary,
