...
OUT_DIR = os.sep.join([
    os.path.dirname(os.path.abspath(__file__)), DEFAULT_INPUT_DIR, 'mayavi',
    'auto'
])
...
def is_valid_rst(string):
    ...
    """
    publisher = docCore.Publisher(
        source_class=docIO.StringInput, destination_class=docIO.StringOutput)
...
class ModuleReference(object):
    ...
    def document_function(self, func_name):
        ...
        if hasattr(self.module, 'test_' + func_name):
            example_code = getsource(getattr(self.module, 'test_' + func_name))
            example_code = """
import numpy
...
%s
        ...
        documentation = document_function(
            func,
            func_name=func_name,
    ...
    def write_doc(self):
        ...
        """
        self.to_document = set([
            name for name, func in getmembers(self.module)
...
""" % self.module.__name__)
        ...
        for submodule, header, filename, title in zip(
                self.sub_modules, self.sub_headers, self.sub_filenames,
                self.sub_modules_titles):

            self.write_doc_submodule(
                filename, title=title, header=header, submodulename=submodule)
            outfile.write('\t%s\n' % filename)
...
''' % self.module.__name__)
        ...
        for func_name in sorted(self.to_document):
            ...
            if (submodulename is not None and
                    not func.__module__ == submodulename):
                continue
...
""",
        ...
        sub_filenames=[
            'mlab_%s.rst' % module.__name__.split('.')[-1]
            for module in sub_modules
        ],
        sub_modules_titles=[
            'Plotting functions', 'Figure handling functions',
            'Figure decoration functions', 'Camera handling functions'
        ],
...
""",
        sub_modules=[sources.__name__, tools.__name__, probe_data.__name__],
        sub_filenames=[
            'mlab_pipeline_sources.rst', 'mlab_pipeline_tools.rst',
