...
class Outline(Module):
    ...
    # `OutlineCornerFilter`.
    outline_filter = Property(
        Instance(
            tvtk.ObjectBase, allow_none=False), record=True)
    ...
    view = View(
        Group(
            ...
            Group(
                'manual_bounds',
                Item(
                    'bounds', enabled_when='manual_bounds'),
                label='Bounds', ),
            Item(
    ...
    _cornered_outline = Instance(
        tvtk.OutlineCornerFilter, args=(), allow_none=False)
