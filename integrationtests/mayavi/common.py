...
def compare_image_with_saved_image(src_img,
    ...
    if allow_resize:
        src_resample = tvtk.ImageResample(
            interpolate=1, interpolation_mode='cubic')
        configure_input(src_resample, src_img)
        img_resample = tvtk.ImageResample(
            interpolate=1, interpolation_mode='cubic')
    ...
    def run(self):
        ...
        try:
            if self.profile:
                ...
                memory_assistant.assertReturnsMemory(
                    self.do_profile, slack=1.0)
            else:
                self.do()
